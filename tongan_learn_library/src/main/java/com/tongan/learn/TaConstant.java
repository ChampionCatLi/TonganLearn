package com.tongan.learn;

import android.graphics.Color;

import androidx.annotation.Keep;


@Keep
public class TaConstant {
    public static final int STUDY_ACTIVITY_CODE = 717;
    public static final String FACE_IS_SUCCESS = "faceIsSuccess";
    public static String TA_KEY_INFO= "info";
    public static String TA_KEY_CAZZ_ID = "clazzId";
    public static String TA_KEY_TYPE = "type";
    public static String TA_KEY_IS_FORCE = "forceRawHead";
    public static String TA_KEY_LOG_ID="logId";
    public static String TA_JSESSION_HEADER = "JSESSIONID=";
    public static String jsessionId = "";
    public static int themColor = Color.parseColor("#2196f3");
    public static int cameraTipsColor = Color.parseColor("#F58609");
    public static final String TONGAN_LMS_URL = "tongan_lms_url";
    public static final String TONGAN_LMS_THEM = "tongan_lms_them";
    public static final String TONGAN_LMS_STATUS = "tongan_lms_status";
    public static final String TONGAN_LMS_CAMERA_TIPS = "tongan_lms_camera_tips";
    public static final String TONGAN_ORIGIN_PHOTO_URL = "m/home/my/a/idphoto.png";
    public static final String TONGAN_FACE_URL = "m/home/my/facerecog/submit";

    public static final String TONGAN_TYPE_RE_SIGN="3";

}